...
begin
  ...
  select l.archive_number, l.status into v_archive_number, v_status
  from public.licenses as l where l.id = p_license_id for update;
  if not found then raise exception 'License not found' using errcode = 'P0002'; end if;
  if v_status = 'archived' then raise exception 'Archived licenses cannot be renewed' using errcode = '23505'; end if;
  update public.licenses as l set issue_date = p_issue_date, expiry_date = p_expiry_date, status = 'active', updated_by = v_actor
